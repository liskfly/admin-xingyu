  ...
  export default {
    ...
    methods: {
      calcRate() {
        ...
        const currentRate = parseFloat((this.$refs.box.clientWidth / this.$refs.box.clientHeight).toFixed(5));
        if (this.appRef) {
          if (currentRate > this.baseProportion) {
            // 表示更宽
            this.scale.width = ((this.$refs.box.clientHeight * this.baseProportion) / this.baseWidth).toFixed(5);
            this.scale.height = (this.$refs.box.clientHeight / this.baseHeight).toFixed(5);
            this.appRef.style.transform = `scale(${this.scale.width}, ${this.scale.height}) translate(-50%, -50%)`;
          } else {
            // 表示更高
            this.scale.height = ((this.$refs.box.clientWidth / this.baseProportion) / this.baseHeight).toFixed(5);
            this.scale.width = (this.$refs.box.clientWidth / this.baseWidth).toFixed(5);
            this.appRef.style.transform = `scale(${this.scale.width}, ${this.scale.height}) translate(-50%, -50%)`;
            this.proportion = ((this.$refs.box.clientWidth / this.baseProportion) / this.baseHeight);
          }
        }
      },
      resize() {
