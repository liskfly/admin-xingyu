  ...
  <div class="p-2">
    <el-card shadow="always" :body-style="{ padding: '8px' }">
      ...
      >
        ...
        <af-table-column prop="ToolsMold" label="编码" fixed> </af-table-column>
        <af-table-column prop="MaterialName" label="描述" fixed>
        </af-table-column>
...
import { moldControl, specControl, iDControll } from "@/api/all";
export default {
  data() {
    return {
      searchText: "",
  ...
  methods: {
      ...
      if (num < 259) {
        arr.map((item, index) => {
          if (num == index && item['刷新日期']) {
            ...
            //     console.log("error" + item.__EMPTY, index);
            //   });
            // if (this.arrTest.some((i) => i.PN_Model == `${item.__EMPTY}`)) {
            //   // console.log('有' + item.__EMPTY,index);
            //   this.readExcelFile(num + 1);
            // } else {
            //   specControl({
            //     productName: item.__EMPTY_1,
            //     useage: 1,
            //     toolsMold: item.__EMPTY,
            //     remark: "",
            //     operationType: "I",
            //   })
            //     .then(() => {
            ...
            //     });
            // }
          } else if (check && num == index) {
            console.log("空" + item["样件编号"], index);
            this.readExcelFile(num + 1);
            check = false;
          }
        });
    ...
    getData() {
      ...
      //   operationType: "Q",
      // }).then((res) => {
      //   this.arrTest = res.data.DataList;
