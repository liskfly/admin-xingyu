      ...
      <div class="table_container">
        ...
        >
          ...
          <el-table-column
            prop="SubItemName"
            label="工艺检查项"
            v-if="getDataText.inspectType == 'WI'"
          >
          </el-table-column>
          <el-table-column
            prop="SubItemSolution"
            label="检查实际值"
            v-if="getDataText.inspectType == 'WI'"
          >
          </el-table-column>
          ...
          <el-table-column prop="InspectStatus" label="状态">
            <template slot-scope="scope" v-if="scope.row.InspectStatus">
              <el-tag
                :type="resultTag1(scope.row.InspectStatus)"
                effect="plain"
                >{{ resultText1(scope.row.InspectStatus) }}</el-tag
              ></template
            >
          </el-table-column>
...
export default {
  ...
  methods: {
    getData() {
      this.startLoading();
      getCheckResults(this.getDataText)
        .then((res) => {
          this.endLoading();
      ...
      if (this.getDataText.inspectType == "WI") {
        ...
        data.forEach((item) => {
          ...
          if (resultMap.has(InspectOrder)) {
            ...
            const itemExists = group.stepItemList.some(
              (el) => el.Name === Name
            );
