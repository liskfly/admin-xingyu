  ...
  <div class="p-2">
    <el-card shadow="always" :body-style="{ padding: '8px' }">
      ...
      >
        ...
        <el-table-column
          prop="examine_productname"
          label="料号"
        ></el-table-column>
        <el-table-column prop="examine_status" label="启用">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.examine_status"
              :active-value="1"
              :inactive-value="0"
      ...
      >
        <el-form :model="addForm" ref="addFormRef" label-width="auto">
          <el-form-item label="料号" prop="examine_productname">
            <el-input
              v-model="addForm.examine_productname"
              placeholder="请输入料号"
            ></el-input
          ></el-form-item>
          <el-form-item label="启用" prop="examine_status">
            <el-switch
              v-model="addForm.examine_status"
              :active-value="1"
              :inactive-value="0"
            ></el-switch
          ></el-form-item>
  ...
  methods: {
    ...
    deleteData(row) {
      this.$confirm("是否确认删除?", "提示", {
        ...
        .then(() => {
          DeleteXYL_BadExamine({ examine_productname: row.examine_productname }).then(
            (res) => {
              if (res.Success) {
                this.$message({
                  message: res.Msg,
